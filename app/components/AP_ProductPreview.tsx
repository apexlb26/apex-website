...
/** One box in the decorative flow diagram. */
...
export default function AP_ProductPreview({ mock }: { mock: ProductMockContent }) {
  const { nav: NAV, summary: SUMMARY } = mock;
  return (
    <div aria-hidden="true" className="flex w-full min-w-0 flex-wrap gap-2.5">
      ...
      <div className="flex min-w-[min(300px,100%)] flex-1 basis-[62%] overflow-hidden rounded-xl border border-px-line bg-white shadow-[0_14px_34px_rgba(10,26,44,.09)]">
        <div className="w-[68px] shrink-0 border-r border-px-line p-2">
          <strong className="block text-[7.5px] font-bold text-px-ink">{mock.appName}</strong>
          <small className="block text-[5px] text-px-muted">{mock.appSubtitle}</small>
        ...
        <div className="min-w-0 flex-1 p-2">
          <div className="flex flex-wrap items-center gap-1.5">
            <strong className="text-[7.5px] font-bold text-px-ink">{mock.flowTitle}</strong>
            <span className="rounded-full bg-emerald-50 px-1.5 py-0.5 text-[5px] font-bold text-emerald-600">{mock.statusLabel}</span>
            <span className="ml-auto flex gap-1">
              <i className="rounded border border-px-line px-1.5 py-0.5 text-[5px] not-italic text-px-muted">{mock.saveLabel}</i>
              <i className="rounded border border-px-line px-1.5 py-0.5 text-[5px] not-italic text-px-muted">{mock.publishLabel}</i>
            </span>
          </div>
          <div className="mt-1 flex gap-2 border-b border-px-line pb-1 text-[5.5px] text-px-muted">
            {mock.tabs.map((tab, i) => <span key={tab} className={i === 0 ? "border-b border-px-cyan pb-1 font-bold text-px-cyanInk" : ""}>{tab}</span>)}
          </div>
          <div className="flex flex-col items-center gap-1.5 py-2.5">
            <span className="rounded-md bg-emerald-500 px-2 py-1 text-[6px] font-bold text-white">{mock.startLabel}</span>
            <span className="block h-2 w-px bg-px-line" />
            <Node label={mock.flowNodes[0] ?? ""} />
            <span className="block h-2 w-px bg-px-line" />
            <Node label={mock.flowNodes[1] ?? ""} />
            <span className="flex w-full items-start justify-center gap-6">
              <span className="flex flex-col items-center gap-1"><i className="block h-2 w-px bg-px-line" /><Node label={mock.flowNodes[2] ?? ""} /></span>
              <span className="flex flex-col items-center gap-1"><i className="block h-2 w-px bg-px-line" /><Node label={mock.flowNodes[3] ?? ""} /></span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="min-w-[min(200px,100%)] flex-1 basis-[34%] rounded-xl border border-px-line bg-white p-2.5 shadow-[0_14px_34px_rgba(10,26,44,.09)]">
        <div className="flex items-center justify-between">
          <strong className="text-[7.5px] font-bold text-px-ink">{mock.summaryTitle}</strong>
          <span className="rounded border border-px-line px-1.5 py-0.5 text-[5px] text-px-muted">{mock.todayLabel}</span>
        </div>
